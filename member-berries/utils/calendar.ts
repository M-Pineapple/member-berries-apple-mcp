 ...
 * @param fromDate Optional start date for search range in ISO format (default: 2 weeks ago)
 * @param toDate Optional end date for search range in ISO format (default: 3 months from now)
    ...
    try {
        ...
        const events = await run((args: { 
            ...
            try {
                ...
                defaultStartDate.setDate(today.getDate() - args.pastDays);  // 2 weeks ago
                
                const defaultEndDate = new Date(today);
                defaultEndDate.setDate(today.getDate() + args.futureDays);  // 3 months ahead
                ...
                // Priority calendar names
                const priorityNames = ["Home", "Work", "Calendar", "iCloud", "BLS.gov Economic News Release Schedule"];
                
                // Process calendars - priority ones first
                let calendarsProcessed = 0;
                
                // First pass: priority calendars
                for (const calender of calendars) {
                    if (events.length >= args.limit || calendarsProcessed >= args.maxCalendars) break;
                    
                    try {
                        const calName = calender.name();
                        if (!priorityNames.includes(calName)) continue;
                        
                        calendarsProcessed++;
                        
                        // Get events using the same working method as original
                ...
                // Second pass: other calendars if we need more events
                if (events.length < args.limit) {
                    for (const calender of calendars) {
                        ...
                        try {
                            const calName = calender.name();
                            if (priorityNames.includes(calName)) continue; // Skip already processed
            ...
            maxCalendars: CONFIG.MAX_CALENDARS,
