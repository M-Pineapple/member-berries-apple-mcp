...
async function createNote(title: string, body: string, folderName: string = 'Member Berries'): Promise<CreateNoteResult> {
    try {
        ...
        const result = await run((title: string, body: string, folderName: string) => {
            ...
            try {
                ...
                if (!targetFolder) {
                    if (folderName === 'Member Berries') {
                        // Try to create the Member Berries folder if it doesn't exist
                        Notes.make({new: 'folder', withProperties: {name: 'Member Berries'}});
                        ...
                        for (let i = 0; i < updatedFolders.length; i++) {
                            if (updatedFolders[i].name() === 'Member Berries') {
